...
final class AnimalDataMapper: AnimalDataMapperProtocol {
    ...
    func map(_ data: AnimalsResponse) -> [AnimalViewObject] {
        data.animals
            .map { AnimalViewObject(
                id: $0.id,
                name: $0.name,
                description: mapDescription($0.description),
                gender: $0.gender == "Male" ? .male : .female,
                age: $0.age,
                distance: mapRawDistance($0.distance),
                publishedAt: mapRawLastSeen($0.published_at),
                photo: $0.photos.isEmpty ? nil : $0.photos[0].medium,
                tags: $0.tags,
                ownersPhone: $0.contact.phone,
                ownersEmail: $0.contact.email,
                color: mapColor($0.colors),
                size: $0.size
            )}
    }
    
    private func mapRawDistance(_ rawDistance: Double) -> String {
        Int(rawDistance.rounded()).description + "m away"
    }
    
    private func mapColor(_ colors: AnimalsResponse.Animal.Colors) -> String {
    ...
    private func mapDescription(_ description: String?) -> String {
    ...
    private func mapRawLastSeen(_ rawData: String) -> String {
